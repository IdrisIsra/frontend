...
var recentlyViewed = {
  ...
  createSlider: function(el, productsAvailable){
    ...
    if(productsAvailable < 2){
    	$('.js-rv-slider .gallery-cell').eq(productsAvailable-1).nextAll().remove();
    } else {
    	$('.js-rv-slider .gallery-cell').eq(productsAvailable-2).nextAll().remove();
    }
    ...
    if ($('#cart_form .tos_agree').length) {
      ...
      $('body').on('click', "#cart_form input[type='submit']", function() {
        if ($(this).parents('form').find('.tos_agree').is(':checked')) {
          ...
          var warning = '<p class="warning animated bounceIn">' + "Satış Sözleşmesini kabul etmeden devam edemezsin." + '</p>';
...
selectCallback = function(variant, selector) {
  ...
  if (variant && variant.available == true) {
    if(variant.price < variant.compare_at_price){
      ...
      $('.savings', $product).html($('#products_product_savings').html() + ' ' + parseInt(((variant.compare_at_price - variant.price) * 100) / variant.compare_at_price) + '% (' + '<span class="money">' + Shopify.formatMoney(variant.compare_at_price - variant.price, $productForm.data('money-format')) + '</span>)');
    } else {
    ...
    var message = variant ? $('#products_product_sold_out').html() : $('#products_product_unavailable').html();
          ...
          $.ajax({
            ...
            success: function (data) {
              $nf.fadeOut("slow", function(){
                $nf.prev('.message').html("Teşekkürler! Stoklar yenilendiğinde seni bilgilendireceğiz!");
              });
            },
            error: function(data) {
...
var collectionSidebarFilter = {
  init: function(){
    ...
    if($('.swiper-container').length > 0){
        ...
        width: '100',
        centeredSlides: true,
        initialSlide: $('.swiper-wrapper .swiper-slide.active').index()
      }); 
    }
  },
        ...
        $.ajax({
          ...
          success: function (data) {
            $cf.fadeOut("slow", function(){
              $cf.prev('.message').html("Mail listemize katıldığın için teşekkür ederiz!");
            });
          },
          error: function(data) {
      ...
      if (globalQuickShopProduct != 'undefined'){
        ...
        $.each( product.collection_handles, function( value, index ) {
          if (this.toString() == 'new'){
            $('.js-new-banner').html($('#collections_general_new').html());
          }
          if (this.toString() == 'pre-order'){
            $('.js-pre-order-banner').html($('#collections_general_pre_order').html());
          }
        });
        ...
        //No variants
          //Sale Banner
            ...
            if (product.compare_at_price > product.price){
             $('.js-sale-banner').html($('#collections_general_sale').html());
            }
          ...
          if (product.available == false){
            $('.js-sold-out').html($('#products_product_sold_out').html());
          }
          ...
          if (product.available == true) {
            ...
            if (product.compare_at_price > product.price) {
              ...
              $('.js-savings').html($('#products_product_savings').html() + ' ' + parseInt(((product.compare_at_price - product.price) * 100) / product.compare_at_price) + '% (' + '<span class="money">' + Shopify.formatMoney(product.compare_at_price - product.price, product.money_format) + '</span>)');
            }

            if (product.price == $('#collections_general_coming_soon').html()){
              $('.js-current-price').html($('#collections_general_coming_soon').html());
            ...
            if (variant == product.variants[i].id.toString()){
              ...
              if (product.variants[i].available == true) {
                ...
                if (product.variants[i].compare_at_price > product.variants[i].price) {
                  ...
                  $('.js-savings').html($('#products_product_savings').html() + ' ' + parseInt(((product.variants[i].compare_at_price - product.variants[i].price) * 100) / product.variants[i].compare_at_price) + '% (' + '<span class="money">' + Shopify.formatMoney(product.variants[i].compare_at_price - product.variants[i].price, product.money_format) + '</span>)');
                }

                if (product.variants[i].price == $('#collections_general_coming_soon').html()){
                  $('.js-current-price').html($('#collections_general_coming_soon').html());
      ...
      success: function (data) {
        ...
        if(quantity > 0 && quantity != updated_quantity) {
          if (updated_quantity == 1) {
            items_left_text = $('#products_product_items_left_count_one').html();
          } else {
            items_left_text = $('#products_product_items_left_count_other').html();
          }
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              if(variant[0] && variant[0].compare_at_price > item.price) {
                cart_items_html += ' <span class="money was_price">' + Shopify.formatMoney(variant[0].compare_at_price, $cart.data('money-format')) + '</span> ';
    ...
    $('body').on('submit', ".product_form", function(e) {
      ...
      $.ajax({
        ...
        error: function(XMLHttpRequest) {
          ...
          $addToCartBtn.removeAttr('disabled').removeClass('disabled');
          $addToCartBtn.find('span').text($('#products_product_add_to_cart').html()).removeClass('zoomOut').addClass('zoomIn');
        }
      });
...
function resizeWindow(){
  if($(window).width() < 768){
    $('#mobile_menu li.sublink > a').attr('href', 'javascript:void(0);');
    $('.collection-template-section .container .sixteen.columns div > br.clear.product_clear, body.search .container .sixteen.columns div > br.clear.product_clear').remove();
  }
}

function productsListControl() {
  if($(window).width() < 768){
  	$('.collection-template-section .container .sixteen.columns div > br.clear.product_clear').remove();
	$('.collection-template-section .one-third').each(function(){
      var swatchesLenght = $(this).find('.collection_swatches').find('a').length;
      if(swatchesLenght > 4){
        $(this).find('.collection_swatches').find('a:nth-child(4)').attr('href', 'javascript:void(0);');
        $(this).find('.collection_swatches').find('a:nth-child(4) span').html('+' + (swatchesLenght - 3)).addClass('more');
      }
    });
  }
}
...
$("#ctb_placeholder").on("ctb_fully_loaded", function(event, p1) {
  if ((p1=='inserted' || p1=='inserted_sticky') && !ctb_adjusted ){
    ...
    $("<div />", {html: '<style>' + '.header.is-absolute,#header.mobile_nav-fixed--true,.sticky_nav{top:' + ctb_height + 'px!important;}' + '@media(max-width: 768px){div.breadcrumb-collection{padding-top:' + ctb_height + 'px;}}' + '</style>'}).appendTo("body");
    $("<div />", {html: '<style>' + '#epb_background{top:0;z-index:9000!important;}#epb_background a{display: block;}' + '</style>'}).appendTo("body")
  }
  $("#epb_close").on( "click", function(){
    $("<div />", {html: '<style>' + '.header.is-absolute,#header.mobile_nav-fixed--true,.sticky_nav{top:0!important;}@media(max-width: 768px){div.breadcrumb-collection{padding-top:0;}}' + '</style>'}).appendTo("body")
  });
});
$("#ctb_placeholder").on("epb_fully_unloaded", function(event, p1) {
    $("<div />", {html: '<style>' + '.header.is-absolute,#header.mobile_nav-fixed--true,.sticky_nav{top:0!important;}@media(max-width: 768px){div.breadcrumb-collection{padding-top:0;}}' + '</style>'}).appendTo("body")
});